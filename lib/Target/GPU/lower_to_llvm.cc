...
widenPointerArithmeticToI64(Value value, Location loc,
    ...
    // With a 32-bit GPU index type, memref pointer extraction lowers as
    // ptrtoint i64 followed by trunc i32.  Recover the untruncated address
    // before rebuilding byte-offset arithmetic for a global-memory pointer.
    if (auto trunc = value.getDefiningOp<LLVM::TruncOp>()) {
        Value original = trunc.getArg();
        if (isIntegerType(original.getType(), 64) &&
            (original.getDefiningOp<LLVM::PtrToIntOp>() ||
             original.getDefiningOp<LLVM::AddOp>())) {
            return original;
        }
    }
...
static std::optional<Value>
reconcileMemRefDescriptorMetadata(Value source,
                                  LLVM::LLVMStructType targetType, Location loc,
                                  PatternRewriter &rewriter) {
    ...
    if (sourceBody.size() != 5 || targetBody.size() != 5 ||
        ...
        !isa<IntegerType>(sourceBody[2]) ||
        !isa<IntegerType>(targetBody[2])) {
        return std::nullopt;
    }

    int64_t sourceRank = 0;
    int64_t targetRank = 0;
    auto sourceSizeArray = dyn_cast<LLVM::LLVMArrayType>(sourceBody[3]);
    auto targetSizeArray = dyn_cast<LLVM::LLVMArrayType>(targetBody[3]);
    if (!sourceSizeArray || !targetSizeArray) {
        return std::nullopt;
    }
    auto sourceIndexType =
        dyn_cast<IntegerType>(sourceSizeArray.getElementType());
    auto targetIndexType =
        dyn_cast<IntegerType>(targetSizeArray.getElementType());
    if (!sourceIndexType || !targetIndexType ||
        !isLLVMArrayOfInteger(sourceBody[3], sourceIndexType.getWidth(),
                              &sourceRank) ||
        !isLLVMArrayOfInteger(sourceBody[4], sourceIndexType.getWidth()) ||
        !isLLVMArrayOfInteger(targetBody[3], targetIndexType.getWidth(),
                              &targetRank) ||
        !isLLVMArrayOfInteger(targetBody[4], targetIndexType.getWidth()) ||
...
class ReconcileNVGPUIndex32CastsPattern
  ...
  public:
    ...
    matchAndRewrite(UnrealizedConversionCastOp op,
        ...
        auto widened = reconcileMemRefDescriptorMetadata(
