...
TEST_F(MLIRGeneratorTest, GenerateMLIRNVVMTmaPrefetchDescriptor) {
    static const std::string kSourceCode = R"""""(
import avelang
import avelang.language as S

@avelang.jit
def tma_prefetch_descriptor_test(global_mem: S.Tensor((16, 16), S.f32)):
    smem_layout = S.make_layout((16, 16), (16, 1))
    desc = S.nvvm.make_tma_descriptor(global_mem, smem_layout)
...
def nvvm_fma_test(dst: S.Tensor((1,), S.f32)):
    ...
    dst[0] = maximum + S.nvvm.fast_rcp(c)
)""""";
...
@avelang.jit
def wgmma_rs_test(a: S.Tensor((1, 4), S.i32), dst: S.Tensor((64,), S.f32)):
    smem_b = S.make_shared((16, 128), S.bf16, 128)
    desc_b = S.nvvm.make_wgmma_descriptor_bits(smem_b, 1, 0, 0, 0)
    acc = S.nvvm.wgmma_init_result(64)
    acc = S.nvvm.wgmma_m64n128k16_f32_bf16_bf16_rs(a[0], desc_b, acc, 0)
    dst[0] = acc[0]
)""""";
...
TEST_F(MLIRGeneratorTest, GenerateMLIRNVVMWgmmaFP8) {
    static const std::string kSourceCode = R"""""(
import avelang
import avelang.language as S

@avelang.jit
def wgmma_fp8_test(dst: S.Tensor((96,), S.f32)):
    smem_a = S.make_shared((64, 32), S.u8, 128)
    smem_b = S.make_shared((32, 192), S.u8, 128)
    desc_a = S.nvvm.make_wgmma_descriptor_bits(smem_a, 1, 0, 0, 0)
    desc_b = S.nvvm.make_wgmma_descriptor_bits(smem_b, 1, 0, 0, 0)
    acc = S.nvvm.wgmma_init_result(96)
    acc = S.nvvm.wgmma_m64n192k32_f32_e4m3_e4m3(desc_a, desc_b, acc, 0)
    acc = S.nvvm.wgmma_m64n192k32_f32_e4m3_e4m3(desc_a, desc_b, acc, 1)
    dst[0] = acc[0]
)""""";
